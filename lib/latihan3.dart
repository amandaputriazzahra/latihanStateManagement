...
abstract class UniversityEvent {} // Abstract class untuk event-event yang terkait dengan universitas.

class FetchUniversities extends UniversityEvent {
  // Class FetchUniversities yang meng-extends UniversityEvent untuk event fetch data universitas.
  final String country; // Variabel untuk menyimpan nama negara.

  FetchUniversities(
      this.country); // Konstruktor untuk inisialisasi nama negara.
}

class UniversitiesBloc extends Bloc<UniversityEvent, List<University>> {
  // Class UniversitiesBloc yang meng-extends Bloc dengan input UniversityEvent dan output List<University>.
  UniversitiesBloc()
      : super(
  ...
  Stream<List<University>> mapEventToState(UniversityEvent event) async* {
    // Method untuk memetakan event menjadi state.
    if (event is FetchUniversities) {
      // Jika event yang diterima adalah FetchUniversities.
      yield await fetchUniversities(event
          .country); // Memancarkan (yield) state baru setelah melakukan fetch data universitas.
    }
  }
  ...
  Future<List<University>> fetchUniversities(String country) async {
    ...
    if (response.statusCode == 200) {
      ...
      return universities; // Mengembalikan list universities sebagai hasil fetch.
    } else {
      throw Exception(
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: BlocProvider(
        create: (context) =>
            UniversitiesBloc(), // Membuat instance UniversitiesBloc saat aplikasi diinisialisasi.
        child:
...
class UniversitiesPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final UniversitiesBloc universitiesBloc = BlocProvider.of<UniversitiesBloc>(
        context); // Mendapatkan instance UniversitiesBloc dari BlocProvider.
      ...
      body: Column(
        ...
        children: [
          BlocBuilder<UniversitiesBloc, List<University>>(
            // Widget BlocBuilder untuk membangun UI berdasarkan state dari UniversitiesBloc.
                  ...
                  onChanged: (String? newValue) {
                    ...
                    if (newValue != null) {
                      universitiesBloc.add(FetchUniversities(
                          newValue)); // Mengirim event FetchUniversities dengan nilai negara yang dipilih.
                    }
                  },
                ),
              );
